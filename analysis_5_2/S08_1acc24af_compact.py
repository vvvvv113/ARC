"""
S08_1acc24af_compact.py

Compact view of task 1acc24af:
  Section 1 — Task description: all training examples + test input/target
              Both input and output rendered at TASK_C per cell (same visual scale).
  Section 2 — 1 human participant (all attempts, multi-line wrap)
  Section 3 — 5 CodeIt success traces (first 5 by seed order)

Derived from S08_state_graph_new.py; only changes:
  1. human_participants[:1]  — one participant only
  2. codeit_success_traces[:5]  — five traces only
  3. Section 1 output rendered with TASK_C per cell (matches input scale)
"""
...
_total_human = len(human_participants)
# ── Compact: show only first participant ──────────────────────────────────────
human_participants = human_participants[:1]
print(f"Human participants (showing 1 of {_total_human} available)")
...
# ── Compact: show only first 5 CodeIt traces ─────────────────────────────────
codeit_success_traces = codeit_success_traces[:5]
...
print(f"Human participants shown: {len(human_participants)}")
print(f"CodeIt success traces shown: {len(codeit_seqs)}  seeds: {codeit_seeds}")
...
DPI            = 130
GH             = 0.72
TASK_C         = 0.12    # cell size for task description grids (both input AND output)
ARROW_IN       = 0.32
ARROW_IN_C     = 0.62
PLACEHOLDER_W  = GH
ROW_PAD        = 0.28
...
# ── Output rendered at TASK_C per cell (same scale as input) ──────────────────
# Row height = tallest of (input, output) at TASK_C per cell.
task_row_hs = [max(len(inp), len(out)) * TASK_C for inp, out, *_ in all_task_rows]
task_sec_h  = sum(task_row_hs) + len(all_task_rows) * (ROW_PAD + 0.32) + 0.50
task_row_w  = max(
    len(inp[0]) * TASK_C + ARROW_IN + len(out[0]) * TASK_C
...
for inp, out, label, in_col, out_col in task_rows_reversed:
    ...
    gh_in  = in_H  * TASK_C
    gw_in  = in_W  * TASK_C
    gh_out = out_H * TASK_C   # output same per-cell scale as input
    gw_out = out_W * TASK_C

    # Row occupies the taller of the two grids; each grid is centered vertically.
    row_h     = max(gh_in, gh_out)
    y_bot_row = y_row - row_h
    y_in_bot  = y_bot_row + (row_h - gh_in)  / 2
    y_out_bot = y_bot_row + (row_h - gh_out) / 2

    ftxt(x0 + gw_in / 2, y_in_bot + gh_in + 0.03,
    ...
    ax_in,  _, _ = add_grid(x0,                    y_in_bot,  inp, gh_in,  border=in_col,  lw=1.0)
    ax_out, _, _ = add_grid(x0 + gw_in + ARROW_IN, y_out_bot, out, gh_out, border=out_col, lw=1.5)
...
ftxt(x0, y_human + human_sec_h - 0.04,
     f"Human success traces — all attempts  (P1 of {_total_human} solved participants)",
...
if codeit_seqs:
    ...
    ftxt(x0, y_codeit + n_c * codeit_row_stride + 0.08,
         f"CodeIt success traces  (n={n_c} of 15)  —  dashed boxes = non-grid intermediate states",
...
out_path = os.path.join(OUT_DIR, f"{TASK_ID}_compact.png")
